            ...
            using (FlowerContext db = new FlowerContext())
            {
                ...
                switch (request)
                {
                    ...
                    case "GET":
                        ...
                        string responseString = JsonConvert.SerializeObject(flowers);
                    ...
                    case "POST":
                        // Преобразуем запрос в список цветов
                        var newFlowers = JsonConvert.DeserializeObject<List<Flower>>(dopString);
                        // Удаляем все старые записи из базы данных
                        db.Flowers.RemoveRange(db.Flowers);
                        // Добавляем новые записи в базу данных
                        db.Flowers.AddRange(newFlowers);
                        // Сохраняем изменения
                        db.SaveChanges();
